    ...
    def load_data(self, paths, mode='train'):
        ...
        for c_dir in c_dir_list:  # handwaving
            ...
            for p_c_dir in p_c_dir_list:  # person01_handwaving_d1_uncomp
                ...
                for j, cur_file in enumerate(filelist):  # image_0257
                    ...
                    frame_np = frame_np[:, :, 0]  #
                    frames_np.append(frame_np)  # (N, 120, 160)
        ...
        frames_np = np.asarray(frames_np) #（ N, 120, 160 )
        data = np.zeros((frames_np.shape[0], self.image_width, self.image_width, 1), dtype=np.float32)
