...
def train(model, ims, real_input_flag, configs, itr):
    ...
    if itr % configs.display_interval == 0:
        ...
        print('training loss: ' + str(cost[0]))
...
def test(model, test_input_handle, configs, save_name):
    ...
    real_input_flag_zero = np.zeros((configs.batch_size, output_length - 1,
                                    ...
                                    configs.img_width // configs.patch_size,
                                    configs.patch_size**2 * configs.img_channel), dtype='float32')

    while not test_input_handle.no_batch_left():
        batch_id = batch_id + 1
        test_ims = test_input_handle.get_batch()
        test_dat = preprocess.reshape_patch(test_ims, configs.patch_size)
        img_gen = model.test(test_dat, real_input_flag_zero)[0]
...
def test_g(model, test_input_handle, configs, save_name):
    """Evaluates a model."""
    print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'test...')
    test_input_handle.begin(do_shuffle=False)
    res_path = os.path.join(configs.gen_frm_dir, str(save_name))
